...
class TSONN():
    ...
    def __init__(self, layers, device):
        ...
        self.model = LegendreKAN_norm(self.layers, self.X.cpu().detach().numpy(), 4,self.device).to(self.device)
    ...
    def train(self, adam_epochs):
        ...
        with open(f'result/{nu_f}_legend.csv', 'a') as log_file:
            ...
            for stage, lr in enumerate(learning_rates):
                ...
                for i in range(start_epoch, min(end_epoch, adam_epochs)):
                    ...
                    if i % 1000 == 0:
                        torch.save(self.model, f'checkpoint/legend_{nu_f}.pth')
